"""
Command-Line Interface for SquiggleCaller: CTC Connectionist Temporal Classification Basecaller.
"""
import argparse
import csv
import json
...
from .agents import SquiggleBasecallerCoordinator

coordinator = SquiggleBasecallerCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="nanopore-sequencing-basecaller", description="SquiggleCaller: CTC Connectionist Temporal Classification Basecaller")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  SQUIGGLECALLER: CTC CONNECTIONIST TEMPORAL CLASSIFICATION BASECALLER")
        print(f"  Domain: Bioinformatics | Standard: Oxford Nanopore Raw Data Standards")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting SquiggleCaller: CTC Connectionist Temporal Classification Basecaller on http://{args.host}:{args.port}")
